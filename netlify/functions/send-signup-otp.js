...
export async function handler(event) {
  ...
  try {
    ...
    const payload = `signup:${normalizedEmail}:${otp}:${expiresAt}:${nonce}`;
    ...
    if (!brevoApiKey) {
      console.log('\n==================================================');
      console.log(`  [KESHAVCODER DEV OTP]`);
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          challengeToken,
          message: 'OTP generated (Dev mode: check terminal console for code)',
        }),
      };
    }

    // Fully mobile-responsive email template without horizontal overflow
    const htmlContent = `
<!DOCTYPE html>
...
`;
    ...
    console.error('[send-signup-otp]', error.message);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
