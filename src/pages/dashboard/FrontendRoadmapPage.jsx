...
export function FrontendRoadmapPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.warn('[FrontendRoadmap error]', err);
      } finally {
  ...
  const handleToggleStep = async (stepId) => {
    ...
    setToggling(true);
    try {
      const updated = await toggleUserRoadmapStep(user.uid, 'frontend', stepId);
  ...
  return (
    <RoadmapViewer
      ...
      subtitle="A structured 21-phase journey from web fundamentals to advanced React performance, testing, and interview readiness."
      steps={steps}
