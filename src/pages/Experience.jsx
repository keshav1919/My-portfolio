...
export default function Experience() {
  return (
    <PageContainer className="inner-page experience-page">
      ...
      <section className="experience-feature"><article><span className="experience-feature__icon"><BriefcaseBusiness /></span><span className="eyebrow">Experience</span><h2>Frontend Developer</h2><strong>1 Year Experience</strong><p>Built responsive frontend pages and reusable interface components with HTML, CSS, JavaScript, Tailwind CSS and React.</p><ul>{responsibilities.map((item) => <li key={item}>{item}</li>)}</ul></article><div><article><span className="experience-feature__icon"><GraduationCap /></span><span className="eyebrow">Education</span><h2>BCA 1st Year</h2><p>Currently pursuing a Bachelor of Computer Applications while continuing to build frontend projects and strengthen development fundamentals.</p></article><article><span className="experience-feature__icon"><Rocket /></span><span className="eyebrow">Next chapter</span><h2>Growing toward full stack development</h2><ul>{goals.map((item) => <li key={item}>{item}</li>)}</ul></article></div></section>
